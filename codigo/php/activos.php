...
/* ===============================
   Parámetros
   =============================== */
$minutosActivo = 30; // ACTIVO = visto en últimos X minutos
$porPagina = 50;
$pagina = isset($_GET['p']) ? max(1, (int)$_GET['p']) : 1;
$offset = ($pagina - 1) * $porPagina;

/* ===============================
   Conteo total de activos
   =============================== */
$sqlTotal = "
    SELECT COUNT(*) AS c
    FROM dispositivo
    WHERE ultima_actualizacion >= (NOW() - INTERVAL {$minutosActivo} MINUTE)
";
$resTotal = $conn->query($sqlTotal);
$rowTotal = $resTotal->fetch_assoc();
$totalActivos = (int)$rowTotal['c'];

$totalPaginas = max(1, ceil($totalActivos / $porPagina));
...
$sql = "
    ...
    WHERE d.ultima_actualizacion >= (NOW() - INTERVAL {$minutosActivo} MINUTE)
    ORDER BY d.ip ASC
...
<body>
  ...
  <div class="container">
    ...
    <h2 class="section-title">
      🟢 Dispositivos activos (últimos <?= (int)$minutosActivo ?> min)
    </h2>
    ...
    <table class="table">
      ...
      <?php if (empty($items)): ?>
        <tr>
          <td colspan="7" class="muted">No hay activos en este período.</td>
        </tr>
      <?php else: ?>
